...
for vm_opts_file in "$IDE_BIN_HOME/studio$BITS.vmoptions" "$HOME/.AndroidStudio1.5/studio$BITS.vmoptions" "$STUDIO_VM_OPTIONS"; do
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=AndroidStudio1.5"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JDK/bin/java" \
  ...
  -Djb.restart.code=88 -Didea.paths.selector=AndroidStudio1.5 \
